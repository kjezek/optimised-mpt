...
for max_height in 3 5 7 9 11; do
  ...
  mkdir $TEMP_DB
  START_TIME=$(date +%s)
  node --max-old-space-size=$HEAP_SIZE bucket-trie-insert.js "${DB_PATH_BUCKET_TRIE}_${max_height}" $PARALLELISM 1 $max_height $INPUT_TRIE
  END_TIME=$(date +%s)
  echo "Total time $((END_TIME - START_TIME))s"
  cp $SPEED_FILE "speed-bucket-trie-update-sample.csv"
done
